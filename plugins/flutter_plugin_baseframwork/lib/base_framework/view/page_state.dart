...
import 'package:flutter_plugin_baseframwork/base_framework/config/router_manager.dart';
import 'package:flutter_plugin_baseframwork/base_framework/utils/image_helper.dart';
import 'package:flutter_plugin_baseframwork/base_framework/view_model/view_state.dart';
import 'package:flutter_plugin_baseframwork/base_framework/wigets/common_emptyview.dart';
import 'base_state.dart';

/*
 *如果是页面，继承 [PageState]
 *如果是view，继承 [WidgetState]
 *确保你的页面名字在项目中唯一，否则一些页面跳转的结果可能非你预期
 *此处扩展功能应该只与page相关
 *@author zln
 *create at 2020/10/27 下午6:08
 */
abstract class PageState extends BaseState with WidgetGenerator,RouteAware,_RouteHandler{
  ...
  Widget switchStatusBar2DarkByList({bool isSetDark = true,@required Widget child,
    ///适配、
    EdgeInsets edgeInsets,bool needSlideOut = false,Function reLoad}){
    if(mCommonEmptyView == null){
      mCommonEmptyView = new CommonEmptyView(mReLoad: reLoad,);
    }
    if(! needSlideOut){
      //不含侧滑退出
      return getNormalPage(isSetDark: isSetDark,child: child,edgeInsets: edgeInsets,
